...
export default function BankSettings() {
  ...
  const inp = 'w-full bg-white border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-300 focus:outline-none focus:border-gray-400 transition-colors font-mono'
  ...
  return (
    <div className="p-8 max-w-xl">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h1 className="text-xl font-semibold text-gray-900 tracking-tight">Bankverbindung</h1>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <label className="text-xs font-medium text-gray-400 block mb-1.5">IBAN</label>
          <input className={inp} placeholder="DE00 0000 0000 0000 0000 00" value={form.bank_iban} onChange={e => f('bank_iban', e.target.value)} />
        </div>
        <div>
          <label className="text-xs font-medium text-gray-400 block mb-1.5">BIC / SWIFT</label>
          <input className={inp} placeholder="XXXXXXXX" value={form.bank_bic} onChange={e => f('bank_bic', e.target.value)} />
        </div>
        <div>
          <label className="text-xs font-medium text-gray-400 block mb-1.5">Kontoinhaber</label>
