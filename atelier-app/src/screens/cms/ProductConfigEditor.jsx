...
function RatingBadge({ rating }) {
  ...
  const Icon = cfg.icon
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${cfg.bgColor} ${cfg.textColor}`}>
      <Icon size={10} />
      {cfg.label}
    </span>
  )
      ...
      {Object.entries(ratingConfig).map(([key, cfg]) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border-2 transition-all bg-transparent ${
              value === key ? `${cfg.bgColor} ${cfg.textColor} border-current` : 'border-gray-100 text-gray-400'
            }`}
          >
            <Icon size={12} />
            {cfg.label}
...
// ── Shared input styles ─────────────────────────────────────────────────────
const inp = 'w-full bg-white border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-300 focus:outline-none focus:border-gray-400'
...
function MaterialForm({ initial = emptyMat, onSave, onCancel }) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-5 space-y-4">
      <h3 className="text-sm font-semibold text-gray-700">{initial.id ? 'Material bearbeiten' : 'Neues Material'}</h3>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          ...
          <div className="flex items-center gap-2">
            <input type="color" value={f.color} onChange={e => s('color', e.target.value)} className="w-10 h-10 rounded-lg border border-gray-200 bg-transparent cursor-pointer" />
        ...
        <div>
          ...
          <button onClick={() => s('available', f.available ? 0 : 1)}
            className={`w-full py-2.5 rounded-lg text-xs font-medium border-0 ${f.available ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
      ...
      <div className="flex gap-3">
        <button onClick={() => valid && onSave(f)} disabled={!valid}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-medium border-0 ${valid ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>
          <Check size={14} /> Speichern
        </button>
        <button onClick={onCancel} className="px-4 py-2.5 rounded-lg text-xs font-medium text-gray-500 bg-gray-100 border-0">Abbrechen</button>
      </div>
    </div>
  )
}
...
function ColorForm({ initial = emptyCol, onSave, onCancel }) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-5 space-y-4">
      <h3 className="text-sm font-semibold text-gray-700">{initial.id ? 'Farbe bearbeiten' : 'Neue Farbe'}</h3>
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className={labelCls}>Schlüssel *</label>
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input type="color" value={f.hex} onChange={e => s('hex', e.target.value)} className="w-10 h-10 rounded-lg border border-gray-200 bg-transparent cursor-pointer" />
...
function SoleForm({ initial = emptySole, onSave, onCancel }) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-5 space-y-4">
      <h3 className="text-sm font-semibold text-gray-700">{initial.id ? 'Sohle bearbeiten' : 'Neue Sohle'}</h3>
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className={labelCls}>Schlüssel *</label>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          ...
          <button onClick={() => s('recommended', f.recommended ? 0 : 1)}
            className={`w-full py-2.5 rounded-lg text-xs font-medium border-0 ${f.recommended ? 'bg-teal-50 text-teal-700' : 'bg-gray-100 text-gray-400'}`}>
      ...
      <div>
        ...
        <p className="text-[10px] text-gray-400 mt-1">Schuhtypen: OXFORD, LOAFER, DERBY, BOOT, SNEAKER, MONK</p>
      </div>
      <div>
      ...
      <div className="space-y-2">
        {items.map(item => (
          mode?.editing?.id === item.id ? (
            ...
            <div key={item.id} className="bg-white border border-gray-100 rounded-xl flex items-center gap-4 px-5 py-3.5 group hover:border-gray-200 transition-all">
              {renderItem(item)}
        ...
        {items.length === 0 && <div className="text-center py-10 text-gray-400 text-sm">Keine Einträge</div>}
      </div>
    </div>
  )
}
...
export default function ProductConfigEditor() {
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="mb-8">
        <h1 className="text-xl font-semibold text-gray-900 tracking-tight">Produkt-Konfiguration</h1>
      ...
      <Section
        ...
        renderItem={item => (
          <>
            <div className="w-12 h-12 rounded-xl flex-shrink-0" style={{ background: `radial-gradient(circle at 35% 35%, ${item.color}bb, ${item.color})` }} />
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-gray-500 mt-0.5">{item.sub} · {item.season || '–'}</p>
              {item.tip && <p className="text-[10px] text-gray-400 mt-1 line-clamp-1">{item.tip}</p>}
            </div>
          </>
        )}
      />
      ...
      <Section
        ...
        renderItem={item => (
          <>
            <div className="w-12 h-12 rounded-xl flex-shrink-0 border border-gray-100" style={{ backgroundColor: item.hex }} />
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-sm font-semibold text-gray-900">{item.name}</p>
                <span className="text-[10px] text-gray-400 font-mono">{item.hex}</span>
                <RatingBadge rating={item.rating} />
                {!item.available && <span className="text-[10px] text-red-500 font-medium">Nicht verfügbar</span>}
              </div>
              {item.pairs_with && <p className="text-[10px] text-gray-500 mt-0.5">Passt zu: {item.pairs_with}</p>}
              {item.tip && <p className="text-[10px] text-gray-400 mt-1 line-clamp-1">{item.tip}</p>}
            </div>
          </>
        )}
      ...
      <Section
        ...
        renderItem={item => (
          <>
            <div className={`w-12 h-12 rounded-xl flex-shrink-0 flex items-center justify-center ${item.recommended ? 'bg-teal-50' : 'bg-gray-100'}`}>
