...
// ── Ampel-Rating Badge ──────────────────────────────────────────────────────
const ratingConfig = {
 good: { label: 'Empfohlen', color: 'bg-black/50', textColor: 'text-black/50', bgColor: 'bg-black/[0.03]', icon: CheckCircle2 },
 neutral: { label: 'Neutral', color: 'bg-black/40', textColor: 'text-black/40', bgColor: 'bg-black/[0.03]', icon: Circle },
 warn: { label: 'Achtung', color: 'bg-black/35', textColor: 'text-black/40', bgColor: 'bg-black/[0.03]', icon: AlertTriangle },
}
...
function RatingBadge({ rating }) {
 ...
 const Icon = cfg.icon
 return (
 <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-light ${cfg.bgColor} ${cfg.textColor}`}>
 <Icon size={10} />
 {cfg.label}
 </span>
 )
...
function RatingPicker({ value, onChange }) {
 ...
 className={`flex items-center gap-1.5 px-3 py-2 text-[11px] font-light border transition-all bg-transparent ${
 value === key ? `${cfg.bgColor} ${cfg.textColor} border-current` : 'border-black/[0.08] text-black/30'
 }`}
 >
 <Icon size={12} />
 {cfg.label}
...
function MaterialForm({ initial = emptyMat, onSave, onCancel }) {
 ...
 <h3 className="text-[9px] text-black/25 uppercase tracking-[0.25em] mb-5 font-light">{initial.id ? 'Material bearbeiten' : 'Neues Material'}</h3>
 ...
 <input type="color" value={f.color} onChange={e => s('color', e.target.value)} className="w-10 h-10 border-b border-black/[0.08] bg-transparent cursor-pointer" />
 ...
 className={`w-full h-10 text-[11px] font-light border-b border-black/[0.08] bg-transparent ${f.available ? 'text-black/50' : 'text-black/30'}`}>
 ...
 <button onClick={onCancel} className="px-6 h-11 text-[11px] text-black/30 hover:text-black/60 bg-transparent border-0 transition-colors font-light">Abbrechen</button>
 </div>
 </div>
 )
}
...
function ColorForm({ initial = emptyCol, onSave, onCancel }) {
 ...
 return (
 <div className="bg-white p-7 space-y-4">
 <h3 className="text-[9px] text-black/25 uppercase tracking-[0.25em] mb-5 font-light">{initial.id ? 'Farbe bearbeiten' : 'Neue Farbe'}</h3>
 <div className="grid grid-cols-3 gap-3">
 <div>
 <label className={labelCls}>Schlüssel *</label>
 ...
 <div className="flex items-center gap-2">
 <input type="color" value={f.hex} onChange={e => s('hex', e.target.value)} className="w-10 h-10 border-b border-black/[0.08] bg-transparent cursor-pointer" />
 ...
 <textarea value={f.tip || ''} onChange={e => s('tip', e.target.value)} rows={2} placeholder="Wann passt diese Farbe am besten?" className="w-full py-3 px-4 border-b border-black/[0.08] text-[13px] bg-transparent outline-none focus:border-black/25 transition-colors font-light text-black/70 placeholder-black/15 resize-y" style={{ fontFamily: 'inherit' }} />
 </div>
 <div>
...
function SoleForm({ initial = emptySole, onSave, onCancel }) {
 ...
 return (
 <div className="bg-white p-7 space-y-4">
 <h3 className="text-[9px] text-black/25 uppercase tracking-[0.25em] mb-5 font-light">{initial.id ? 'Sohle bearbeiten' : 'Neue Sohle'}</h3>
 <div className="grid grid-cols-3 gap-3">
 <div>
 <label className={labelCls}>Schlüssel *</label>
 ...
 <div className="grid grid-cols-3 gap-3">
 <div>
 <label className={labelCls}>Aufpreis (EUR)</label>
 ...
 <p className="text-[10px] text-black/25 mt-1 font-light">Schuhtypen: OXFORD, LOAFER, DERBY, BOOT, SNEAKER, MONK</p>
 </div>
 <div>
 <label className={labelCls}>Beschreibung</label>
 <textarea value={f.description || ''} onChange={e => s('description', e.target.value)} rows={2} className="w-full py-3 px-4 border-b border-black/[0.08] text-[13px] bg-transparent outline-none focus:border-black/25 transition-colors font-light text-black/70 placeholder-black/15 resize-y" style={{ fontFamily: 'inherit' }} />
 </div>
 <div>
 <label className={labelCls}>Empfehlungstext (Info-Box in App)</label>
 <textarea value={f.tip || ''} onChange={e => s('tip', e.target.value)} rows={2} className="w-full py-3 px-4 border-b border-black/[0.08] text-[13px] bg-transparent outline-none focus:border-black/25 transition-colors font-light text-black/70 placeholder-black/15 resize-y" style={{ fontFamily: 'inherit' }} />
 </div>
 <div className="flex gap-3">
 <button onClick={() => valid && onSave(f)} disabled={!valid}
 className="px-8 h-11 border border-black text-black text-[11px] bg-transparent hover:bg-black hover:text-white transition-all duration-300 uppercase tracking-[0.2em] font-light disabled:opacity-30 flex-1 flex items-center justify-center gap-2">
...
function Section({ title, subtitle, items, Form, emptyForm, onAdd, onUpdate, onDelete, renderItem }) {
 ...
 <div>
 <h2 className="text-[9px] text-black/25 uppercase tracking-[0.25em] mb-2 font-light">{title}</h2>
 <p className="text-[13px] text-black/30 font-light">{subtitle}</p>
 </div>
 {!mode && (
 ...
 <p className="text-[13px] text-black/25 font-light">Keine Einträge</p>
 </div>
 )}
 </div>
 </div>
...
export default function ProductConfigEditor() {
 ...
 <span className="text-[10px] text-black/30 font-light">{cfg.label}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <span className="text-[10px] text-black/25 font-mono font-light">{item.hex}</span>
 <RatingBadge rating={item.rating} />
 {!item.available && <span className="text-[10px] text-black/30 font-light">Nicht verfügbar</span>}
 </div>
 {item.pairs_with && <p className="text-[10px] text-black/30 mt-0.5 font-light">Passt zu: {item.pairs_with}</p>}
 {item.tip && <p className="text-[10px] text-black/25 mt-1 line-clamp-1 font-light">{item.tip}</p>}
 </div>
 </>
 )}
 ...
 renderItem={item => (
 <>
 <div className={`w-12 h-12 flex-shrink-0 flex items-center justify-center bg-black/[0.02]`}>
 <span className="text-lg">{item.recommended ? '⭐' : '👟'}</span>
 </div>
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2">
 <p className="text-[13px] font-light text-black/70">{item.label}</p>
 <span className="text-[10px] text-black/25 font-light">{item.sub}</span>
 <RatingBadge rating={item.rating} />
 {item.recommended === 1 && <span className="text-[10px] text-black/30 font-light">Empfohlen</span>}
 </div>
 <p className="text-[10px] text-black/30 mt-0.5 font-light">
 Kategorien: {item.categories} · {item.price_extra > 0 ? `+ EUR ${item.price_extra}` : 'Inklusive'}
 </p>
 {item.tip && <p className="text-[10px] text-black/25 mt-1 line-clamp-1 font-light">{item.tip}</p>}
 </div>
 </>
