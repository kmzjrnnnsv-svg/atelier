...
function FAQForm({ initial = emptyForm, onSave, onCancel, saving }) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-6 space-y-4 mb-4">
      ...
      {/* Question */}
      <div>
        <label className="block text-xs font-medium text-gray-400 mb-1.5">Frage *</label>
        <textarea
          value={form.question}
      ...
      {/* Answer */}
      <div>
        <label className="block text-xs font-medium text-gray-400 mb-1.5">Antwort *</label>
        <textarea
          value={form.answer}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-gray-400 mb-1.5">Reihenfolge</label>
          <input
            type="number"
            min="0"
      ...
      <div className="flex gap-3 pt-1">
        ...
        <button
          onClick={onCancel}
          className="px-4 py-2.5 rounded-lg text-xs font-medium bg-gray-100 text-gray-500 hover:bg-gray-200 border-0 transition-all"
        >
          Abbrechen
        </button>
      </div>
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-semibold text-gray-900 tracking-tight">FAQ & Support</h1>
          </div>
          <p className="text-xs text-gray-500">{faqs.length} Einträge · in der App unter Hilfe & Support sichtbar</p>
        </div>
        {mode === null && (
          <button
            onClick={() => setMode('add')}
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {categories.map(cat => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium border-0 transition-all ${
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          ...
          <button onClick={() => setMode('add')} className="mt-3 text-xs text-gray-900 underline bg-transparent border-0">
              ...
              <div className="flex items-center gap-3 p-4">
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-[8px] text-gray-400">#{faq.sort_order}</span>
                    </div>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button
                    ...
                    className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center border-0 transition-all"
                  >
                    <Pencil size={12} className="text-gray-500" />
                  </button>
                  <button
                    onClick={() => handleDelete(faq.id)}
                    className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-red-100 flex items-center justify-center border-0 transition-all"
                  >
