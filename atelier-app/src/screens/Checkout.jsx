      ...
      {STEPS.map((label, i) => (
        <div key={label} className="flex items-center" style={{ flex: i < STEPS.length - 1 ? '1 1 0' : 'none' }}>
          <div className="flex flex-col items-center gap-1">
            ...
            <span className={`text-[8px] uppercase tracking-wide whitespace-nowrap ${i === current ? 'text-black font-bold' : 'text-gray-400'}`}>
              {label}
            </span>
          </div>
          {i < STEPS.length - 1 && (
            <div className={`h-px flex-1 mx-2 mb-4 transition-all ${i < current ? 'bg-black' : 'bg-gray-200'}`} />
          )}
        </div>
      ))}
    </div>
...
function AddressForm({ title, value, onChange, sameToggle }) {
  ...
  const inp = 'w-full border border-gray-200 rounded-xl px-4 py-3 text-sm placeholder-gray-300 focus:outline-none focus:border-black transition-colors'
  return (
    <div>
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 transition-all text-left ${
        selected ? 'border-black bg-black/3' : 'border-gray-100 bg-white'
      }`}
    >
      ...
      <div className="flex flex-col h-full bg-white overflow-hidden">
        <div className="flex-1 overflow-y-auto px-6 pt-16 pb-8">
          <div className="flex flex-col items-center text-center mb-6">
            ...
            <h1 className="text-xl font-bold text-black mb-1">Bestellung aufgegeben</h1>
          ...
          <div className="bg-teal-50 border border-teal-200 rounded-2xl p-5 mb-4">
            ...
            <div className="space-y-2.5">
              {[
                ...
                <div key={label} className="flex items-center justify-between gap-3">
                  <span className="text-[9px] uppercase tracking-widest text-gray-400 flex-shrink-0">{label}</span>
                  <span className={`text-right break-all ${big ? 'text-base font-bold text-teal-700' : mono ? 'text-[11px] font-mono font-semibold text-gray-800' : 'text-[11px] font-semibold text-gray-800'}`}>
                    {value}
                  </span>
                </div>
              ))}
            ...
            <div className="mt-4 pt-4 border-t border-teal-200">
              <p className="text-[9px] uppercase tracking-widest text-gray-500 mb-2">Verwendungszweck (pflicht)</p>
          ...
          <div className="bg-gray-50 rounded-2xl p-4 mb-4">
            <p className="text-[9px] uppercase tracking-widest text-gray-400 mb-3">Bestellung</p>
            ...
            {chosenAccessories.map(a => (
              <div key={a.id} className="flex justify-between">
                ...
                <span className="text-[11px] text-gray-500">{a.price}</span>
              </div>
            ))}
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-3.5 rounded-2xl border-2 border-black text-sm font-bold text-black bg-white"
            >
              Meine Bestellungen
            </button>
            <button
              onClick={() => navigate('/collection')}
              className="flex-1 py-3.5 rounded-2xl bg-black text-sm font-bold text-white border-0"
            >
              Weiter shoppen
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-2 border-b border-gray-100 flex-shrink-0">
        ...
        <span className="text-sm font-bold tracking-wide text-black">Checkout</span>
        <div className="w-9" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-4">
        ...
        {step === 1 && (
          <div>
            <button
              ...
              className="w-full flex items-center gap-3 p-4 rounded-2xl border-2 mb-5 transition-all text-left border-0 bg-transparent"
              style={{ border: sameBilling ? '2px solid #000' : '2px solid #e5e7eb' }}
            >
              <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                sameBilling ? 'bg-black border-black' : 'border-gray-300'
              }`}>
        ...
        {step === 2 && (
          <div>
            <h2 className="text-base font-bold text-black mb-1">Zubehör & Accessoires</h2>
            <p className="text-[11px] text-gray-400 mb-5">Ergänzen Sie Ihre Bestellung mit passendem Zubehör.</p>
            <div className="space-y-3">
        ...
        {step === 3 && (
          <div>
            <h2 className="text-base font-bold text-black mb-5">Bestellübersicht</h2>

            {/* Shoe */}
            <div className="bg-gray-50 rounded-2xl p-4 mb-4">
              <p className="text-[9px] uppercase tracking-widest text-gray-400 mb-3">Schuh</p>
            ...
            {chosenAccessories.length > 0 && (
              <div className="bg-gray-50 rounded-2xl p-4 mb-4">
                <p className="text-[9px] uppercase tracking-widest text-gray-400 mb-3">Zubehör</p>
                {chosenAccessories.map(a => (
            ...
            {/* Addresses */}
            <div className="bg-gray-50 rounded-2xl p-4 mb-4">
              <p className="text-[9px] uppercase tracking-widest text-gray-400 mb-2">Lieferadresse</p>
      ...
      <div className="px-5 pt-3 border-t border-gray-100 flex-shrink-0" style={{ paddingBottom: 'max(env(safe-area-inset-bottom, 0px), 20px)' }}>
        {step < 3 ? (
          <button
            onClick={handleNext}
            disabled={!canNext}
            className={`w-full py-4 rounded-2xl flex items-center justify-center gap-2 font-bold text-sm uppercase tracking-widest transition-all border-0 ${
              canNext ? 'bg-black text-white active:scale-[.98]' : 'bg-gray-100 text-gray-300'
            }`}
          >
          ...
          <button
            onClick={handlePlace}
            disabled={placing}
            className="w-full py-4 rounded-2xl flex items-center justify-center gap-2 bg-black text-white font-bold text-sm uppercase tracking-widest active:scale-[.98] border-0 disabled:opacity-60"
          >
            {placing ? (
              <><div className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" /> Wird verarbeitet…</>
            ) : (
