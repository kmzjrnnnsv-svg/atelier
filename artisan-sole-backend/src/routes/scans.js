...
const ML_SCRIPTS = join(new URL('.', import.meta.url).pathname, '..', '..', '..', 'artisan-sole-ml', 'scripts')

// Resolve Python binary: prefer venv over system python3
// Searches: PYTHON_PATH env → ../artisan-sole-ml/.venv/bin/python3 → ~/ml-venv/bin/python3 → python3
        ...
        if (hasAllImages) {
          try {
            const saveScript = new URL(
              '../../../artisan-sole-ml/scripts/save_real_scan.py',
              import.meta.url
            ).pathname
...
router.post('/lidar-measurements', authenticate, async (req, res) => {
  ...
  const scriptPath = new URL(
    '../../../artisan-sole-ml/scripts/process_lidar.py',
    import.meta.url
  ).pathname
  ...
  if (depCheck.status !== 0) {
    ...
    return res.status(500).json({
      ...
      detail: 'Bitte "pip install -r requirements.txt" im artisan-sole-ml Verzeichnis ausführen.',
    })
  }
      ...
      child.on('close', (code) => {
        ...
        if (code !== 0) {
          ...
          const detail = isModuleError
            ? 'Python-Abhängigkeiten fehlen. Bitte "pip install -r requirements.txt" im artisan-sole-ml Verzeichnis ausführen.'
            : isTimeout
...
router.post('/photogrammetry', authenticate, async (req, res) => {
  ...
  if (depCheck.status !== 0) {
    ...
    return res.status(500).json({
      error: `Python-Abhängigkeit fehlt: ${missing}`,
      detail: 'Bitte "pip install opencv-python numpy scipy scikit-learn" im artisan-sole-ml Verzeichnis ausführen.',
    })
  }
