...
export default class AtelierPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        try {
          ...
          new Notice("Artisan Sole: Scans synchronisiert!");
        } catch (e: any) {
    ...
    this.addCommand({
      ...
      callback: async () => {
        try {
          ...
          new Notice("Artisan Sole: Bestellungen synchronisiert!");
        } catch (e: any) {
          new Notice(`Fehler: ${e.message}`);
        }
      },
    ...
    this.addCommand({
      ...
      callback: async () => {
        try {
          ...
          new Notice("Artisan Sole: Schuhe synchronisiert!");
        } catch (e: any) {
          new Notice(`Fehler: ${e.message}`);
        }
      },
    ...
    this.addCommand({
      ...
      callback: async () => {
        try {
          ...
          new Notice("Artisan Sole: Artikel synchronisiert!");
        } catch (e: any) {
          new Notice(`Fehler: ${e.message}`);
        }
      },
    ...
    this.addRibbonIcon("database", "Artisan Sole Sync", () => this.runSync());

    this.setupAutoSync();

    console.log("Artisan Sole Plugin geladen");
  }
  ...
  onunload() {
    ...
    console.log("Artisan Sole Plugin entladen");
  }
  ...
  async runSync() {
    if (!this.settings.email || !this.settings.password) {
      new Notice("Artisan Sole: Bitte zuerst E-Mail und Passwort in den Einstellungen eingeben.");
      return;
    }
    try {
      await this.sync?.syncAll();
    } catch (e: any) {
      console.error("Artisan Sole sync error:", e);
    }
  }
}
