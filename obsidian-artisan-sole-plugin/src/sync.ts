...
export class AtelierSync {
  ...
  async syncAll(): Promise<void> {
    const notice = new Notice("Artisan Sole: Synchronisiere...", 0);
    let synced = 0;
    try {
      await this.syncUsers();
      synced++;
      notice.setMessage(`Artisan Sole: ${synced}/8 synchronisiert...`);
      ...
      notice.hide();
      new Notice("Artisan Sole: Sync abgeschlossen!");
    } catch (e: any) {
      notice.hide();
      new Notice(`Artisan Sole Sync-Fehler: ${e.message}`);
      throw e;
    }
  }
  ...
  async syncDashboard(): Promise<void> {
    ...
    const content = [
      "---",
      "tags: [artisan-sole, dashboard]",
      "---",
      "",
      "# Artisan Sole Dashboard",
      "",
